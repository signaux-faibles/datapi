drop materialized view if exists v_summaries;

drop materialized view if exists v_roles;
create table if not exists v_roles  as
...
create index if not exists idx_v_roles_roles
    on v_roles using gin (roles);

create index if not exists idx_v_roles_siret
    on v_roles (siren);

drop materialized view if exists v_naf;
create table if not exists v_naf as
...
create index if not exists idx_v_naf_n1
    on v_naf (code_n1);

create index if not exists idx_v_naf_n5
    on v_naf (code_n5);

drop materialized view if exists v_last_procol;
create table if not exists v_last_procol as
...
create unique index if not exists idx_v_last_procol_siren
    on v_last_procol (siren);



drop materialized view if exists v_last_effectif;
create table if not exists v_last_effectif as
...
drop materialized view if exists v_hausse_urssaf;
create table if not exists v_hausse_urssaf as
...
drop materialized view if exists v_etablissement_raison_sociale;
create table if not exists v_etablissement_raison_sociale as
...
create index if not exists idx_entreprise_raison_sociale
    on v_etablissement_raison_sociale using gin (siret gin_trgm_ops, raison_sociale gin_trgm_ops);

drop materialized view if exists v_diane_variation_ca;
create table if not exists v_diane_variation_ca as
...
create unique index if not exists idx_v_diane_variation_ca_siren
    on v_diane_variation_ca (siren);

drop materialized view if exists v_apdemande;
create table if not exists v_apdemande as
SELECT e.siret,
...
create unique index if not exists idx_v_apdemande_siret
    on v_apdemande (siret);


drop materialized view if exists v_alert_etablissement;
create table if not exists v_alert_etablissement as
...
create unique index if not exists idx_v_alert_etablissement_siret
    on v_alert_etablissement (siret);


drop materialized view if exists v_alert_entreprise;
create table if not exists v_alert_entreprise as
...
create index if not exists idx_v_alert_entreprise_siren
    on v_alert_entreprise (siren);


create table if not exists v_summaries as
...
create index if not exists idx_v_summaries_order_raison_sociale
    on v_summaries (raison_sociale, siret);

create index if not exists idx_v_summaries_raison_sociale
    on v_summaries using gin (siret gin_trgm_ops, raison_sociale gin_trgm_ops);

create index if not exists idx_v_summaries_roles
    on v_summaries using gin (roles);

create index if not exists idx_v_summaries_score
    on v_summaries (valeur_score desc, siret asc, siege asc, effectif asc, effectif_entreprise asc, code_departement asc,
                    ...
                    asc, first_red_list_entreprise asc, has_delai asc)
    where (alert <> 'Pas d''alerte'::text);

create index if not exists idx_v_summaries_siren
    on v_summaries (siren);

create index if not exists idx_v_summaries_siret
