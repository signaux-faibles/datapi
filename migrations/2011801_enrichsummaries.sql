...
-- create or replace function f_siret_perm(
-- 	in roles_user text[], 
-- 	in username text,
--   in siret varchar(9)
-- ) returns table (
...
-- (permissions($1, r.roles, a.first_list, e.departement, f.siren is not null)).visible,
-- (permissions($1, r.roles, a.first_list, e.departement, f.siren is not null)).in_zone,
-- (permissions($1, r.roles, a.first_list, e.departement, f.siren is not null)).score,
-- (permissions($1, r.roles, a.first_list, e.departement, f.siren is not null)).urssaf,
-- (permissions($1, r.roles, a.first_list, e.departement, f.siren is not null)).dgefp,
-- (permissions($1, r.roles, a.first_list, e.departement, f.siren is not null)).bdf,
-- from etablissement0 e
...
-- ) as $$
-- 	select 
--     et.siret, et.siren, en.raison_sociale,
--     et.commune, d.libelle as libelle_departement,
...
--     di.chiffre_affaire, di.arrete_bilan, di.variation_ca, di.resultat_expl, 
--     ef.effectif, n.libelle_n5, n.libelle_n1, et.code_activite,
--     coalesce(ep.last_procol, 'in_bonis') as last_procol,
--     case when f_siret_perm($1, $9, et.siret).dgefp then coalesce(ap.ap, false) end activite_partielle,
--     case when f_siret_perm($1, $9, et.siret).dgefp then ap.heure_consomme end as apconso_heure_consomme,
--     case when f_siret_perm($1, $9, et.siret).dgefp then ap.montant end as apconso_montant,
--     case when f_siret_perm($1, $9, et.siret).urssaf then u.dette[1] > u.dette[2] or u.dette[2] > u.dette[3] end as hausse_urssaf,
--     case when f_siret_perm($1, $9, et.siret).urssaf then u.dette[1] end as dette_urssaf,
--     case when f_siret_perm($1, $9, et.siret).score then s.alert end,
--     count(*) over () as nb_total,
...
--     p.visible, p.in_zone, f.id is not null as followed,
--     fe.siren is not null as followed_entreprise,
--     et.siege, g.raison_sociale as raison_sociale_groupe,
--     ti.code_commune is not null as territoire_industrie,
--     f.comment, f.category, f.since,
--     f_siret_perm($1, $9, et.siret).urssaf,
--     f_siret_perm($1, $9, et.siret).dgefp,
--     f_siret_perm($1, $9, et.siret).score,
--     f_siret_perm($1, $9, et.siret).bdf
-- 	from etablissement0 et
--     inner join entreprise0 en on en.siren = et.siren
--     inner join f_etablissement_permissions($1, $9) p on et.id = p.id
--     inner join v_etablissement_raison_sociale etrs on etrs.id_etablissement = et.id
