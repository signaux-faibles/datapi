...
func (cds *CampaignDetails) Tuple() []interface{} {
	...
	*cds = append(*cds, &cd)
	return []interface{}{
		&c.ID, &c.Libelle, &c.WekanDomainRegexp, &c.DateEnd, &c.DateCreate,
		&ce.ID, &ce.CampaignID, &ce.Siret,
		&cea.ID, &cea.CampaignEtablissementID, &cea.Username, &cea.DateAction, &cea.Action,
	}
}
...
func selectCampaignDetailsWithCampaignIDAndZone(ctx context.Context, id CampaignID, zone []string) (CampaignDetails, error) {
	sql := `select c.id as id_campaign, c.libelle, c.wekan_domain_regexp, c.date_end, c.date_create,
      ce.id as id_campaign_etablissement, ce.id_campaign, ce.siret,
      cea.id as id_campaign_etablissement_action, cea.id_campaign_etablissement, cea.username, cea.date_action, cea.action
    from campaign c
